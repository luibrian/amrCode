...
%things. Starts by assuming that the optional walls are actually there.

%OUTPUTS:
%mapStruct contains information about the map obstacles and boundaries
%   mapFour = map, including boundaries, in (n+m)x4 format. m = #opt walls
%   mapSixteen = map, excluding boundaries, in (n+m-4)x16 format. Adds some
...
%mapEight= nx8 array that contains only the walls, not the
%   boundaries
